...
} from "@ads/store/services/adminRoomPromotionsApi";
import {RoomPromotionStatus} from "@ads/models/room-promotion";
import {toast} from "sonner";

export default function AdminRoomPromotionDetailsPage() {
    const router = useRouter();
    const params = useParams();
    const promotionId = Number(params.id);

    const {data, isLoading, error} = useGetRoomPromotionByIdQuery(promotionId);
    const [approvePromotion, {isLoading: isApproving}] = useApproveRoomPromotionMutation();
    const [denyPromotion, {isLoading: isDenying}] = useDenyRoomPromotionMutation();
    const [cancelPromotion, {isLoading: isCanceling}] = useCancelRoomPromotionMutation();

    const isPending = data?.status === RoomPromotionStatus.PENDING;
    const isApproved = data?.status === RoomPromotionStatus.APPROVED;
    ...
    const handleApprove = async () => {
        ...
        try {
            ...
            toast.success("Promotion approved. The payment has been captured and the room was bumped to the top.");
        } catch {
                ...
                <div className="@container/main flex flex-1 flex-col gap-2">
                    <div className="flex flex-col gap-4">
                        <div className="mx-4 mt-4 flex flex-wrap gap-2">
                            ...
                            {isPending && (
                                <PromotionReasonModal mode="room-deny" onSubmit={handleDeny} disabled={isMutating}/>
                            )}
                            ...
                            {(isApproved || (isPending && data.cancelRequested)) && (
                                <PromotionReasonModal
                                    mode={isApproved ? "room-cancel-approved" : "room-cancel-pending"}
                                    onSubmit={handleCancel}
                                    disabled={isMutating}
                                />
                            )}
                            {!data.chatRoomArchived && (
                                <ActionButton onClick={handleOpenRoom} variant="default"
                        ...
                        <RoomPromotionDetails className="m-4 mt-0" data={data} isAdmin={true}/>
                    </div>
                </div>
            </div>
        </div>
