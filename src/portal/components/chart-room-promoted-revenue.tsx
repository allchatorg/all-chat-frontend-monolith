...
import {useGetRoomPromotedRevenueDailyQuery} from "@ads/store/services/adminRoomPromotionsApi"
import {Skeleton} from "@ads/components/ui/skeleton"

export const description = "Room promotions revenue chart"

// Violet matches the room-promotion series in chart-bar-revenue.tsx
const chartConfig = {
    roomPromotedRevenue: {
...
export function ChartRoomPromotedRevenue() {
    ...
    // Fetch captured room-promotion revenue from the API
    const {data, isLoading, isError} = useGetRoomPromotedRevenueDailyQuery({
    ...
    const chartData = React.useMemo(() => {
        ...
        return data.dailyRevenue.map((item) => ({
            date: item.date,
            roomPromotedRevenue: item.revenue
        }))
    }, [data])
    ...
    if (isLoading) {
        return (
            <Card className="@container/card">
                <CardHeader>
                    <CardTitle>Room Promotions Revenue</CardTitle>
            ...
            <Card className="@container/card">
                ...
                <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
                    <div className="flex h-[250px] items-center justify-center text-muted-foreground">
                        Failed to load room promotion revenue data
                    </div>
                </CardContent>
            </Card>
        )
            ...
            <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
                ...
                >
                    <AreaChart data={chartData} margin={{top: 12}}>
                        <defs>
                            <linearGradient id="fillRoomPromotedRevenue" x1="0" y1="0" x2="0" y2="1">
                                <stop offset="5%" stopColor="var(--color-roomPromotedRevenue)" stopOpacity={0.8}/>
                                <stop offset="95%" stopColor="var(--color-roomPromotedRevenue)" stopOpacity={0.1}/>
                            </linearGradient>
                        </defs>
                        ...
                        <Area
                            ...
                            type="natural"
                            fill="url(#fillRoomPromotedRevenue)"
                            stroke="var(--color-roomPromotedRevenue)"
                        />
                    </AreaChart>
                </ChartContainer>
            </CardContent>
